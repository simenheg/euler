...
(defun lychrel-p (n &optional (i 50))
  (incf n (parse-integer (reverse (write-to-string n))))
  (or (= i 0)
      (let ((str (write-to-string n)))
        (unless (string= str (reverse str))
          (lychrel-p n (- i 1))))))

(loop for i below 10000 count (lychrel-p i)) ; => 249
